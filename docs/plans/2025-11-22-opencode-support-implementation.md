...
> **For Claude:** REQUIRED SUB-SKILL: Use austinpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add full austinpowers support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
- Reference: `.codex/austinpowers-codex` (lines 40-74)
...
- Reference: `.codex/austinpowers-codex` (lines 97-136)
 ...
 * @param {string} sourceType - 'personal' or 'austinpowers' for namespacing
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/austinpowers-codex` (lines 212-280)
 ...
 * (personal skills override austinpowers skills).
 *
 * @param {string} skillName - Name like "austinpowers:brainstorming" or "my-skill"
 * @param {string} austinpowersDir - Path to austinpowers skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, austinpowersDir, personalDir) {
    // Strip austinpowers: prefix if present
    const forceAustinpowers = skillName.startsWith('austinpowers:');
    const actualSkillName = forceAustinpowers ? skillName.replace(/^austinpowers:/, '') : skillName;

    // Try personal skills first (unless explicitly austinpowers:)
    if (!forceAustinpowers && personalDir) {
    ...
    // Try austinpowers skills
    if (austinpowersDir) {
        const austinpowersPath = path.join(austinpowersDir, actualSkillName);
        const austinpowersSkillFile = path.join(austinpowersPath, 'SKILL.md');
        if (fs.existsSync(austinpowersSkillFile)) {
            return {
                skillFile: austinpowersSkillFile,
                sourceType: 'austinpowers',
                skillPath: actualSkillName
            };
        }
    }
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/austinpowers-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/austinpowers-codex` (add import at top)
...
Run: `node -c .codex/austinpowers-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/austinpowers-codex
...
**Files:**
- Modify: `.codex/austinpowers-codex` (lines 40-74)
...
Run: `.codex/austinpowers-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/austinpowers-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/austinpowers-codex` (lines 16-38, approximately)
...
Run: `.codex/austinpowers-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/austinpowers.js`
...
/**
 * Austinpowers plugin for OpenCode.ai
 *
...
const austinpowersSkillsDir = path.join(homeDir, '.config/opencode/austinpowers/skills');
const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

/**
 * OpenCode plugin entry point
 */
export const AustinpowersPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
Run: `ls -l .opencode/plugin/austinpowers.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/austinpowers.js
...
**Files:**
- Modify: `.opencode/plugin/austinpowers.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "austinpowers:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > austinpowers)
          const resolved = skillsCore.resolveSkillPath(
            skill_name,
            austinpowersSkillsDir,
            personalSkillsDir
          );
...
Run: `node -c .opencode/plugin/austinpowers.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the austinpowers and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const austinpowersSkills = skillsCore.findSkillsInDir(
            austinpowersSkillsDir,
            'austinpowers',
            3
          );
          ...
          const allSkills = [...personalSkills, ...austinpowersSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install austinpowers skills to ~/.config/opencode/austinpowers/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'austinpowers:';
...
```javascript
    'session.started': async () => {
      // Read using-austinpowers skill content
      const usingAustinpowersPath = skillsCore.resolveSkillPath(
        'using-austinpowers',
        austinpowersSkillsDir,
        personalSkillsDir
      );

      let usingAustinpowersContent = '';
      if (usingAustinpowersPath) {
        const fullContent = fs.readFileSync(usingAustinpowersPath.skillFile, 'utf8');
        // Strip frontmatter
        const lines = fullContent.split('\n');
        let inFrontmatter = false;
        let frontmatterEnded = false;
        ...
        usingAustinpowersContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- Austinpowers skills: \`austinpowers:skill-name\` (from ~/.config/opencode/austinpowers/skills/)
...
`;
      ...
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/austinpowers')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/austinpowers && git pull` to update austinpowers.' :
        '';
...
**Below is the full content of your 'austinpowers:using-austinpowers' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**

${usingAustinpowersContent}
...
### 1. Install Austinpowers Skills

```bash
# Clone austinpowers skills to OpenCode config directory
mkdir -p ~/.config/opencode/austinpowers
git clone https://github.com/obra/austinpowers.git ~/.config/opencode/austinpowers
```

### 2. Install the Plugin

The plugin is included in the austinpowers repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/austinpowers/.opencode/plugin/austinpowers.js`
...
ln -s ~/.config/opencode/austinpowers/.opencode/plugin/austinpowers.js .opencode/plugin/austinpowers.js
```
...
```
use use_skill tool with skill_name: "austinpowers:brainstorming"
```
...
Personal skills override austinpowers skills with the same name.
...
1. Check plugin file exists: `ls ~/.config/opencode/austinpowers/.opencode/plugin/austinpowers.js`
2. Check OpenCode logs for errors
3. Verify Node.js is installed: `node --version`

### Skills not found

1. Verify skills directory exists: `ls ~/.config/opencode/austinpowers/skills`
...
- Documentation: https://github.com/obra/austinpowers
```
...
Austinpowers works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
Run: `.codex/austinpowers-codex use-skill austinpowers:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/austinpowers-codex bootstrap | head -30`
...
ls -l .opencode/plugin/austinpowers.js
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/austinpowers.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/austinpowers-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/austinpowers-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/austinpowers.js` created with tools and hooks
