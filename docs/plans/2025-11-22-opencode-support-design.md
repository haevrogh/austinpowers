...
Add full austinpowers support for OpenCode.ai using a native OpenCode plugin architecture that shares core functionality with the existing Codex implementation.

## Background

OpenCode.ai is a coding agent similar to Claude Code and Codex. Previous attempts to port austinpowers to OpenCode (PR #93, PR #116) used file-copying approaches. This design takes a different approach: building a native OpenCode plugin using their JavaScript/TypeScript plugin system while sharing code with the Codex implementation.
...
2. **Platform-Specific Wrappers**
   - Codex: CLI script (`.codex/austinpowers-codex`)
   - OpenCode: Plugin module (`.opencode/plugin/austinpowers.js`)

3. **Skill Directories**
   - Core: `~/.config/opencode/austinpowers/skills/` (or installed location)
   - Personal: `~/.config/opencode/skills/` (shadows core skills)

### Code Reuse Strategy

Extract common functionality from `.codex/austinpowers-codex` into shared module:
...
```javascript
{
  ...
  schema: z.object({
    skill_name: z.string().describe('Name of skill (e.g., "austinpowers:brainstorming")')
  }),
...
1. **Inject using-austinpowers content**
   - Full content of the using-austinpowers skill
...
```javascript
// .opencode/plugin/austinpowers.js
const skillsCore = require('../../lib/skills-core');
const path = require('path');
const fs = require('fs');
const { z } = require('zod');

export const AustinpowersPlugin = async ({ client, directory, $ }) => {
  const austinpowersDir = path.join(process.env.HOME, '.config/opencode/austinpowers');
  const personalDir = path.join(process.env.HOME, '.config/opencode/skills');

  return {
    'session.started': async () => {
      const usingAustinpowers = await readSkill('using-austinpowers');
      const skillsList = await findAllSkills();
      const toolMapping = getToolMappingInstructions();

      return {
        context: `${usingAustinpowers}\n\n${skillsList}\n\n${toolMapping}`
      };
    },
...
├── .codex/
│   ├── austinpowers-codex        # UPDATED: Use skills-core
│   ├── austinpowers-bootstrap.md
│   └── INSTALL.md
├── .opencode/
│   ├── plugin/
│   │   └── austinpowers.js       # NEW: OpenCode plugin
...
1. Create `lib/skills-core.js`
   - Extract frontmatter parsing from `.codex/austinpowers-codex`
...
2. Update `.codex/austinpowers-codex` to use shared core
...
1. Create `.opencode/plugin/austinpowers.js`
