...
This fix should dramatically improve austinpowers skills compliance and should reduce the chances of Claude entering its native plan mode unintentionally.
...
**Using-austinpowers workflow graph intercepts EnterPlanMode**
...
Changed `async: true` to `async: false` in hooks.json. When async, the hook could fail to complete before the model's first turn, meaning using-austinpowers instructions weren't in context for the first message.
...
The `austinpowers-codex` bootstrap CLI, Windows `.cmd` wrapper, and related bootstrap content file have been removed. Codex now uses native skill discovery via `~/.agents/skills/austinpowers/` symlink, so the old `use_skill`/`find_skills` CLI tools are no longer needed.
...
The synchronous SessionStart hook blocked the TUI from entering raw mode on Windows, freezing all keyboard input. Running the hook async prevents the freeze while still injecting austinpowers context.
...
- Windows doesn't respect shebangs, so directly invoking the extensionless `austinpowers-codex` script triggered an "Open with" dialog. All invocations now prefixed with `node`.
...
Austinpowers for OpenCode now uses OpenCode's native `skill` tool instead of custom `use_skill`/`find_skills` tools. This is a cleaner integration that works with OpenCode's built-in skill discovery.

**Migration required:** Skills must be symlinked to `~/.config/opencode/skills/austinpowers/` (see updated installation docs).
...
**Strengthened using-austinpowers skill for explicit skill requests**
...
The underlying skills (`austinpowers:brainstorming`, `austinpowers:executing-plans`, `austinpowers:writing-plans`) remain available for Claude to invoke autonomously. This change prevents confusion when Claude would invoke a command that just redirects to a skill anyway.
...
Fixed a confusing pattern where Claude would invoke a skill via the Skill tool, then try to Read the skill file separately. The `using-austinpowers` skill now explicitly states that the Skill tool loads skill content directly—no need to read files.

- Added "How to Access Skills" section to `using-austinpowers`
- Changed "read the skill" → "invoke the skill" in instructions
- Updated slash commands to use fully qualified skill names (e.g., `austinpowers:brainstorming`)
...
**Skill priority in using-austinpowers**
...
- **Rationalizations table** in using-austinpowers - Scannable format including new entries: "I need more context first", "Let me explore first", "This feels productive"
...
- **OpenCode Bootstrap Refactor**: Switched from `chat.message` hook to `session.created` event for bootstrap injection
  ...
  - Explicitly tells the model that using-austinpowers is already loaded to prevent redundant skill loading
...
- **OpenCode Support**: Native JavaScript plugin for OpenCode.ai
  ...
  - Three-tier skill priority: project > personal > austinpowers
...
- Optimized austinpowers bootstrap to eliminate redundant skill execution. The `using-austinpowers` skill content is now provided directly in session context, with clear guidance to use the Skill tool only for other skills. This reduces overhead and prevents the confusing loop where agents would execute `using-austinpowers` manually despite already having the content from session start.
...
- Added unified `austinpowers-codex` script with bootstrap/use-skill/find-skills commands
- Cross-platform Node.js implementation (works on Windows, macOS, Linux)
- Namespaced skills: `austinpowers:skill-name` for austinpowers skills, `skill-name` for personal
- Personal skills override austinpowers skills when names match
...
- Updated terminology: "Austinpowers skills" instead of "Core skills"

### Files Added
- `.codex/INSTALL.md` - Installation guide for Codex users
- `.codex/austinpowers-bootstrap.md` - Bootstrap instructions with Codex adaptations
- `.codex/austinpowers-codex` - Unified Node.js executable with all functionality

**Note:** Codex support is experimental. The integration provides core austinpowers functionality but may require refinement based on user feedback.

## v3.2.3 (2025-10-23)

### Improvements

**Updated using-austinpowers skill to use Skill tool instead of Read tool**
...
### Files Changed
- Updated: `skills/using-austinpowers/SKILL.md` - Changed tool references from Read to Skill

## v3.2.2 (2025-10-21)

### Improvements

**Strengthened using-austinpowers skill against agent rationalization**
...
### Files Changed
- Updated: `skills/using-austinpowers/SKILL.md` - Added three layers of enforcement to prevent skill-skipping rationalization
...
- Added `austinpowers:code-reviewer` agent to plugin's `agents/` directory
- Agent provides systematic code review against plans and coding standards
- Previously required users to have personal agent configuration
- All skill references updated to use namespaced `austinpowers:code-reviewer`
- Fixes #55

### Files Changed
- New: `agents/code-reviewer.md` - Agent definition with review checklist and output format
- Updated: `skills/requesting-code-review/SKILL.md` - References to `austinpowers:code-reviewer`
- Updated: `skills/subagent-driven-development/SKILL.md` - References to `austinpowers:code-reviewer`
...
- All internal skill references now use `austinpowers:` namespace prefix
- Updated format: `austinpowers:test-driven-development` (previously just `test-driven-development`)
...
- **Fixed command syntax in README** (#44) - Updated all command references to use correct namespaced syntax (`/austinpowers:brainstorm` instead of `/brainstorm`). Plugin-provided commands are automatically namespaced by Claude Code to avoid conflicts between plugins.
...
# Austinpowers v2.0.0 Release Notes

## Overview

Austinpowers v2.0 makes skills more accessible, maintainable, and community-driven through a major architectural shift.

The headline change is **skills repository separation**: all skills, scripts, and documentation have moved from the plugin into a dedicated repository ([obra/austinpowers-skills](https://github.com/obra/austinpowers-skills)). This transforms austinpowers from a monolithic plugin into a lightweight shim that manages a local clone of the skills repository. Skills auto-update on session start. Users fork and contribute improvements via standard git workflows. The skills library versions independently from the plugin.
...
**The biggest change:** Skills no longer live in the plugin. They've been moved to a separate repository at [obra/austinpowers-skills](https://github.com/obra/austinpowers-skills).

**What this means for you:**

- **First install:** Plugin automatically clones skills to `~/.config/austinpowers/skills/`
...
1. Your old `~/.config/austinpowers/.git` will be backed up to `~/.config/austinpowers/.git.bak`
2. Old skills will be backed up to `~/.config/austinpowers/skills.bak`
3. Fresh clone of obra/austinpowers-skills will be created at `~/.config/austinpowers/skills/`

### Removed Features

- **Personal austinpowers overlay system** - Replaced with git branch workflow
- **setup-personal-austinpowers hook** - Replaced by initialize-skills.sh
...
- Clones obra/austinpowers-skills on first run
...
- `AUSTINPOWERS_SKILLS_ROOT` set to `~/.config/austinpowers/skills`
...
- Removed obsolete setup-personal-austinpowers call from session-start
...
- Prominent link to austinpowers-skills repo
...
**Removed:**
- `skills/` directory (82 files) - Now in obra/austinpowers-skills
- `scripts/` directory - Now in obra/austinpowers-skills/skills/using-skills/
- `hooks/setup-personal-austinpowers.sh` - Obsolete

**Modified:**
- `hooks/session-start.sh` - Use skills from ~/.config/austinpowers/skills
- `commands/brainstorm.md` - Updated paths to AUSTINPOWERS_SKILLS_ROOT
- `commands/write-plan.md` - Updated paths to AUSTINPOWERS_SKILLS_ROOT
- `commands/execute-plan.md` - Updated paths to AUSTINPOWERS_SKILLS_ROOT
...
- PR #2: Personal austinpowers overlay system (later replaced)
...
```bash
# In Claude Code
/plugin marketplace add obra/austinpowers-marketplace
/plugin install austinpowers@austinpowers-marketplace
```
...
1. **Backup your personal skills** (if you have any):
   ```bash
   cp -r ~/.config/austinpowers/skills ~/austinpowers-skills-backup
   ```

2. **Update the plugin:**
   ```bash
   /plugin update austinpowers
   ```
...
- Skills repository is now at https://github.com/obra/austinpowers-skills
...
**Full Changelog:** https://github.com/obra/austinpowers/compare/dd013f6...main
**Skills Repository:** https://github.com/obra/austinpowers-skills
**Issues:** https://github.com/obra/austinpowers/issues
