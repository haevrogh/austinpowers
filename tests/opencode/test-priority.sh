...
# Verifies that skills are resolved with correct priority: project > personal > austinpowers
...
# 1. Create in austinpowers location (lowest priority)
mkdir -p "$HOME/.config/opencode/austinpowers/skills/priority-test"
cat > "$HOME/.config/opencode/austinpowers/skills/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Austinpowers version of priority test skill
---
# Priority Test Skill (Austinpowers Version)

This is the AUSTINPOWERS version of the priority test skill.

PRIORITY_MARKER_AUSTINPOWERS_VERSION
EOF
...
if [ -f "$HOME/.config/opencode/austinpowers/skills/priority-test/SKILL.md" ]; then
    echo "  [PASS] Austinpowers version exists"
else
    echo "  [FAIL] Austinpowers version missing"
    exit 1
fi
...
# Test 2: Test that personal overrides austinpowers
echo ""
echo "Test 2: Testing personal > austinpowers priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides austinpowers)"
elif echo "$output" | grep -qi "PRIORITY_MARKER_AUSTINPOWERS_VERSION"; then
    echo "  [FAIL] Austinpowers version loaded instead of personal"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|austinpowers" | head -10
fi

# Test 3: Test that project overrides both personal and austinpowers
echo ""
echo "Test 3: Testing project > personal > austinpowers priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION"; then
    ...
    echo "  [FAIL] Austinpowers version loaded instead of project"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
...
# Test 4: Test explicit austinpowers: prefix bypasses priority
echo ""
echo "Test 4: Testing austinpowers: prefix forces austinpowers version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load austinpowers:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_AUSTINPOWERS_VERSION"; then
    echo "  [PASS] austinpowers: prefix correctly forces austinpowers version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] austinpowers: prefix did not force austinpowers version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
